...
import { IPostLikedRPC, IPostSavedRPC, ITopicQueryRPC } from "../interfaces";
import { Topic, topicSchema } from "../model/topic";
  ...
  async findById(id: string): Promise<Topic | null> {
    try {
      const { data } = await axios.get(
        `${this.topicServiceUrl}/rpc/topics/${id}`
      );
  ...
  async findByIds(ids: Array<string>): Promise<Array<Topic>> {
    try {
      const { data } = await axios.post(
        `${this.topicServiceUrl}/rpc/topics/list-by-ids`,
        { ids }
      );
...
export class PostLikedRPC implements IPostLikedRPC {
  ...
  async hasLikedId(userId: string, postId: string): Promise<boolean> {
    try {
      const { data } = await axios.post(
        `${this.postLikedServiceUrl}/rpc/has-liked`,
        { userId, postId }
      );
      return data.data;
    } catch (error) {
  ...
  async listPostIdsLiked(
    ...
    try {
      const { data } = await axios.post(
        `${this.postLikedServiceUrl}/rpc/list-post-ids-liked`,
        { userId, postIds }
      );
      return data.data;
    } catch (error) {
...
export class PostSavedRPC implements IPostSavedRPC {
  ...
  async hasSavedId(userId: string, postId: string): Promise<boolean> {
    try {
      const { data } = await axios.post(
        `${this.postSavedServiceUrl}/rpc/has-saved`,
        { userId, postId }
      );
      return data.data;
    } catch (error) {
  ...
  async listPostIdsSaved(
    userId: string,
    postIds: string[]
  ): Promise<Array<string>> {
    try {
      const { data } = await axios.post(
        `${this.postSavedServiceUrl}/rpc/list-post-ids-saved`,
        { userId, postIds }
      );
      return data.data;
    } catch (error) {
