...
import { RedisPubSub } from "@shared/components/redis-pubsub";
...
export class RedisTopicConsumer {
  ...
  subcriber() {
    RedisPubSub.getInstance().subscribe(EvtPostCreated, (msg: string) => {
      const data = JSON.parse(msg);
      const evt = PostCreatedEvent.from(data);
      this.handledPostCreated(evt);
    });

    RedisPubSub.getInstance().subscribe(EvtPostDeleted, (msg: string) => {
      const data = JSON.parse(msg);
